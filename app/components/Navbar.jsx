  ...
  return (
      ...
      <div className="container_cust   md:w-[1674px]    md:justify-center  relative md:mx-auto">
              ...
              <div className="left_para absolute md:block hidden">
                ...
                <div className="main_nav w-[1655px] ">
            ...
            <Link href="/">
              <Image src={Union} alt={Union} width={0} height={0}  className="block md:absolute md:left-20   md:top-3"/>
            </Link>
          ...
          <div className="devneed hidden md:block absolute  right-1/4 top-1/2 
          -translate-y-[25px] ">
              <div className="animated-border-box-glow "></div>
              <div className=" px-4 font-bold text-white py-2 animated-border-box font-ox text-[20px]">Dev Needed? 🖥️</div>
          </div>
      ...
      <Container1 className={`menu_mobile md:hidden flex  flex-col !px-0    `}>
        <div className={`flex-1 flex px-6 py-5 pb-3 items-center justify-between  ${isOpen ? "bg-white" : "bg-transparent"} `}>
          <h1 className={`font-pira  text-4xl transition-all duration-500  ${isOpen ? " text-[#ff2221]" : "text-white"}`}>RASHA</h1>
          <div className={`transition-all duration-500 p-1 rounded-md ${isOpen ? "bg-white " : "bg-[#ff2221]"} `} onClick={() => setIsOpen(!isOpen)}>
            {isOpen ? <X className="size-7  text-[#ff2221]"/>  : <Menu className="size-7  text-white"/>}
            
          </div>
        </div>
        
        <div className={`bg-white flex-1    ${isOpen ? "block border-t-2 border-[#ff2221]" : "hidden"}`}>
